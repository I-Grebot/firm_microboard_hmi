...
//__eds__ uint16_t can_buffers[8][CAN_DMA_BUFFERS] __attribute__((eds, space(dma), space(ymemory), aligned(CAN_DMA_BUFFERS*16)));
uint16_t can_buffers[8][CAN_DMA_BUFFERS] __attribute__((aligned(CAN_DMA_BUFFERS*16)));
//#endif
...
//    #elif (defined(__dsPIC33E__))
        DMAPWC              = 0; // Clear write collision flags
        DMARQC              = 0; // Clear read collision flags
//    #endif
...
//    #elif (defined(__dsPIC33E__))
        DMA0STAH = __builtin_dmapage(&can_buffers);
        DMA0STAL = __builtin_dmaoffset(&can_buffers);
//    #endif
...
//    #endif
    ...
    DMA2CNT = 7            ; // Transfers 8 words at a timetime

//    #if (defined(__dsPIC33F__))
//        DMA0STA = __builtin_dmaoffset(&can_buffers);
//    #elif (defined(__dsPIC33E__))
        DMA2STAH = __builtin_dmapage(&can_buffers);
...
void can_init(void) {
    ...
    ECAN1_TX_RPN      = 0x0E         ; // 0x0E : C1TX for dsPIC33E
...
void can_isr_config(void) {
    ...
    C1INTEbits.WAKIE  = 0;
    C1INTEbits.IVRIE  = 0;
    C1INTEbits.TBIE   = 0; // Enable Transmit Interrupts
...
void can_send(can_msg_t * message) {
    ...
    switch(message->pid) {
        ...
        case CAN_PID_LOW :
            ...
            while((0xF7F7 | C1TR23CON) == 0xFFFF); // FIXME Sometimes the programm freezes here when no CAN msg has been acked -> might wana watchdog this
    ...
    //can_buffer = (__eds__ uint16_t) (can_buffers + buffer_no);
...
/*
 * CAN RX Buffer processing function
 * Check and process N buffers, useful if only 1
 * message can be processed at a time.
 * can_process_all_buffers() is a basic inline which
 * can be used to process all buffers.
 */
void can_process_n_first_buffers(uint8_t nb_buffer) {
  ...
  buf_idx = CAN_DMA_RX_START;

  // Continue to process until C1RXFULn is not empty
  while(C1RXFUL1 && nb_buffer) {   

    // Check to see if buf_idx bit is set
    buf_mask = (1 << buf_idx);
    if(C1RXFUL1 & buf_mask) {
          ...
          // Decrement buffer counter so only N RX can be handled
          // at a time. User must care about RX buffer overflow then
          nb_buffer--;
      }   

    // Cheap modulo on buf_idx
    // Rollover, back to RX_START
    // Particularly useful if a flag of a smaller bit was set
    // during the processing!
    buf_idx++;
    if(buf_idx > CAN_DMA_BUFFERS) {
      buf_idx = CAN_DMA_RX_START;
    }
  }
}
