...
#define CS_LOW()  SD_CSN = 0	/* MMC CS = L */
#define CS_HIGH() SD_CSN = 1	/* MMC CS = H */
#define CD	(_RB4)	/* Card detected   (yes:true, no:false, default:true) */
...
void power_off (void)
{
	...
	;					/* Turn off socket power (Nothing to do) */
        power_on(); // Yannick Hack
                    // without power_off the sd card is never ready
                    // with power_off the LCD screen cannot use SPI anymore
                    // so with power_on then power_off both SPI devices work fine
}

void reset_fatfs_spi(void)
{
    power_off();
}
